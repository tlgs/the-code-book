...
def _non_zero_random_bytes(n: int) -> bytes:
    candidates = [x.to_bytes(1, "big") for x in range(1, 256)]
    seq = [secrets.choice(candidates) for _ in range(n)]
    return b"".join(seq)
...
def _pkcs1v15(n: int, e: int, M: bytes) -> bytes:
    ...
    PS = _non_zero_random_bytes(k - len(M) - 3)
